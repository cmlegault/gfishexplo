...
res1 <- run_retro_mults(asap.fname = my.asap.fname,
                       n.peels     = n.peels,
                       year.range  = c(2004,2004),
                       cmult.vals  = 1,
                       mmult.vals  = 1,
                       save.files  = F)
write.csv(res1,file="witch_retro_res1.csv",row.names=F)

my.year.range <- c(2010, 2010)
my.mults <- c(1.5,2,2.5,3,3.5,4,4.5,5)
...
res2 <- run_retro_mults(asap.fname=my.asap.fname,
                        ...
                        mmult.vals=1,
                        save.files=F)
write.csv(res2,file="witch_retro_res2.csv",row.names=F)
...
res3 <- run_retro_mults(asap.fname=my.asap.fname,
                        ...
                        mmult.vals=my.mults,
                        save.files=F)
write.csv(res3,file="witch_retro_res3.csv",row.names=F)

# output
res.table <- rbind(res1,res2,res3)
write.csv(res.table,file="witch_retro_res_table.csv",row.names=F)
